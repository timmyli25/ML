...
def main(unused_argv):
  ...
  test_data = np.array(testdf.iloc[:,3:25], dtype=np.float32)
  test_id = np.array(testdf.iloc[:,0:3], dtype=np.int32)
  ...
  train_data = np.array(traindf.iloc[:,3:25],dtype=np.float32)
  train_id = np.array(traindf.iloc[:,0:3], dtype=np.int32)
  ...
  with open('predictions.csv', 'w') as csvfile:
      ...
      for result in predict_results:
          row = list(result['id']) + list(result['features']) + list(result['logits'])
          writer.writerow(row)
