...
def simpleNN(features, labels, mode):
    ...
    flat_dense3 = tf.reshape(dense3, [-1,1])
    dropout = tf.layers.dropout(
        inputs=flat_dense3, rate=0.2, training=mode == tf.estimator.ModeKeys.TRAIN)
...
def main(unused_argv):
  ...
  test_data = np.array(testdf.iloc[:,1:25], dtype=np.float32)
  test_id = np.array(testdf.iloc[:,0], dtype=np.int32)
  ...
  train_data = np.array(traindf.iloc[:,1:25],dtype=np.float32)
  train_id = np.array(traindf.iloc[:,0], dtype=np.int32)
  ...
  with open('predictions.csv', 'w') as csvfile:
      ...
      writer.writerow(['Id', 'groupId', 'matchId', 'assists', 'boosts', 'damageDealt',\
       'DBNOs','headshotKills', 'heals', 'killPlace', 'killPoints', 'kills','killStreaks', 'longestKill', \
       'maxPlace', 'numGroups', 'revives','rideDistance', 'roadKills', 'swimDistance', 'teamKills','vehicleDestroys', \
