...
def print_error_types(model, test_headlines_binary, test_y, test_headlines_raw):
    ...
    for i in range(len(test_headlines_binary)):
        ...
        if result == 1 and prediction == 0:
            false_negative += 1
            #print('False Negative: {}'.format(raw_headline))
            total_true += 1
...
def main():
    df = pd.read_csv('ES_master_events.csv'.format(CONTRACT))
    df = df.dropna()
    ...
    full_train_x = nltk(list(train_positive_x) + list(train_negative_x))
    full_train_y = list(train_positive_y) + list(train_negative_y)
    full_test_x = nltk(list(test_positive_x) + list(test_negative_x))
    ...
    full_train_x_binaries = vectorizer.fit_transform(full_train_x).toarray()
    #full_test_x_binaries = vectorizer.transform(full_test_x[:5000]).toarray()
    full_test_x_binaries = vectorizer.transform(full_test_x).toarray()
    clf = BernoulliNB()

    #clf.fit(full_train_x_binaries,full_train_y)
    #score = clf.score(full_test_x_binaries, full_test_y)
    #print(score)
